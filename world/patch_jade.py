...
# Jade's real head: Rodin bust (from the 4 turnaround frames) mounted on the procedural body; procedural head + hair hidden once it loads
one("dHair=new THREE.Mesh(new THREE.SphereGeometry(.345,16,12,Math.PI*.72,Math.PI*1.56),HRM);dHair.position.set(0,.08,0);dHair.scale.set(1.04,1,1.02);HAIR.add(dHair);",
    "dHair=new THREE.Mesh(new THREE.SphereGeometry(.345,16,12,Math.PI*.72,Math.PI*1.56),HRM);dHair.position.set(0,.08,0);dHair.scale.set(1.04,1,1.02);HAIR.add(dHair);"
    "GL.load('models/jade-bust.glb?v=4',function(gl){var b=gl.scene,bb=new THREE.Box3().setFromObject(b),sz=bb.getSize(new THREE.Vector3()),sc=1.15/sz.y;b.scale.setScalar(sc);bb.setFromObject(b);b.position.set(-(bb.min.x+bb.max.x)/2,.5-bb.min.y,-(bb.min.z+bb.max.z)/2);b.traverse(function(m){if(m.isMesh){m.castShadow=true;if(m.material){m.material.roughness=.85;m.material.metalness=0}}});D.add(b);dh.visible=false;HAIR.visible=false;D.userData.bust=b},undefined,function(){});")
